...
export const VIEW_TYPE_BRAIN_ATLAS = "brain-atlas-view";
 ...
 *  sessions, independent of its assigned lobe. Pan/zoom/hover/click/drag are plain canvas + DOM event
 *  handling — no WebGL, no continuous render loop — so it stays cheap at rest and scales to large
 *  vaults without a GPU dependency. */
export class BrainAtlasView extends ItemView {
	private plugin: BrainAtlasPlugin;
	...
	private draggingNode: BrainNode | null = null;
	private lastPointer = { x: 0, y: 0 };
	private refreshTimer: number | null = null;
	private pulsePhase = 0;

	constructor(leaf: WorkspaceLeaf, plugin: BrainAtlasPlugin) {
		super(leaf);
		this.plugin = plugin;
	}

	getViewType(): string {
		return VIEW_TYPE_BRAIN_ATLAS;
	}
	...
	async onOpen(): Promise<void> {
		...
		container.empty();
		container.addClass("brain-atlas-view");

		const toolbar = container.createDiv({ cls: "brain-atlas-toolbar" });
		this.buildToolbar(toolbar);

		const stage = container.createDiv({ cls: "brain-atlas-stage" });
		this.canvas = stage.createEl("canvas", { cls: "brain-atlas-canvas" });
		const ctx = this.canvas.getContext("2d");
		if (!ctx) throw new Error("Brain Atlas: 2D canvas context unavailable.");
		this.ctx = ctx;

		this.tooltipEl = stage.createDiv({ cls: "brain-atlas-tooltip" });
		this.tooltipEl.hide();

		this.legendEl = container.createDiv({ cls: "brain-atlas-legend" });
		this.buildLegend();
		...
		this.registerInterval(
			window.setInterval(() => {
				if (!this.plugin.settings.idlePulse) return;
	...
	private buildToolbar(toolbar: HTMLElement): void {
		...
		const linksToggle = toolbar.createEl("label", { cls: "brain-atlas-toggle" });
		...
		const regions = toolbar.createDiv({ cls: "brain-atlas-region-toggles" });
		const allRegions: BrainRegion[] = ["frontal", "parietal", "temporal", "occipital", "cerebellum", "stem"];
		for (const region of allRegions) {
			const chip = regions.createEl("button", { cls: "brain-atlas-chip", text: region });
			this.updateChipState(chip, region);
			chip.addEventListener("click", async () => {
				const enabled = this.plugin.settings.enabledRegions;
		...
		const recenter = toolbar.createEl("button", { cls: "brain-atlas-recenter", text: "Recenter" });
		recenter.addEventListener("click", () => this.resetCamera());
	}

	private updateChipState(chip: HTMLElement, region: BrainRegion): void {
		chip.toggleClass("is-active", this.plugin.settings.enabledRegions[region]);
	...
	private buildLegend(): void {
		...
		for (const entry of kindLegend()) {
			const item = this.legendEl.createDiv({ cls: "brain-atlas-legend-item" });
			const swatch = item.createSpan({ cls: "brain-atlas-legend-swatch" });
			swatch.setCssStyles({ backgroundColor: kindColor(entry.kind, this.plugin.settings.palette) });
	...
	private rebuild(): void {
		this.graph = buildBrainGraph(this.app, this.plugin.settings);
		this.positions = layoutBrainNodes(this.graph.nodes, this.plugin.settings.pinnedNodePositions);
	...
	private registerCanvasEvents(): void {
		this.canvas.addEventListener("pointerdown", (e) => {
			...
			this.draggingNode = found?.node ?? null;
			...
			if (this.isPointerDown) {
				...
				if (this.isDragging) {
					...
					if (this.draggingNode) {
						const path = this.draggingNode.file.path;
						const current = this.positions.get(path) ?? { x: 0.5, y: 0.5 };
						const worldDx = dx / this.camera.zoom;
						const worldDy = dy / this.camera.zoom;
						this.positions.set(path, { x: clamp01(current.x + worldDx / WORLD_W), y: clamp01(current.y + worldDy / WORLD_H) });
					} else {
		...
		const endDrag = (e: PointerEvent) => {
			if (this.isPointerDown) {
				if (!this.isDragging) {
					...
					const pos = this.positions.get(this.draggingNode.file.path);
					if (pos) {
						this.plugin.settings.pinnedNodePositions[this.draggingNode.file.path] = { x: pos.x, y: pos.y };
						void this.plugin.saveSettings();
						new Notice(`Brain Atlas: pinned "${this.draggingNode.file.basename}".`);
					}
				}
			}
		...
		this.canvas.addEventListener("contextmenu", (e) => {
			...
			const found = this.hitTest(e.clientX - rect.left, e.clientY - rect.top);
			if (!found) return;
			const path = found.node.file.path;
			const isPinned = Boolean(this.plugin.settings.pinnedNodePositions[path]);
			const menu = new Menu();
			...
			if (isPinned) {
				menu.addItem((item) =>
					item
						.setTitle("Unpin position")
						.setIcon("pin-off")
						.onClick(async () => {
							delete this.plugin.settings.pinnedNodePositions[path];
							await this.plugin.saveSettings();
							this.rebuild();
						})
				);
	...
	private positionTooltip(found: ScreenNode | undefined, px: number, py: number): void {
		...
		const pinned = Boolean(this.plugin.settings.pinnedNodePositions[node.file.path]);
		this.tooltipEl.empty();
		this.tooltipEl.createDiv({ cls: "brain-atlas-tooltip-title", text: node.file.basename });
		this.tooltipEl.createDiv({
			cls: "brain-atlas-tooltip-meta",
			text: `${node.kind} · ${regionLabel(node.region)} · ${node.degree} link${node.degree === 1 ? "" : "s"}${pinned ? " · pinned" : ""}`,
		});
		this.tooltipEl.show();
	...
	private openNode(node: BrainNode, e: PointerEvent): void {
		const action = this.plugin.settings.clickAction;
		if (action === "preview") {
	...
	private drawLobeTints(outlinePath: Path2D): void {
		...
		const anchors = {
			...
			stem: [0.62, 0.92, 0.12],
		} as Record<BrainRegion, [number, number, number]>;
	...
	private computeScreenNodes(): ScreenNode[] {
		...
		for (const node of this.graph.nodes) {
			if (!enabled[node.region] && !this.plugin.settings.pinnedNodePositions[node.file.path]) continue;
	...
	private drawEdges(): void {
		...
		const shouldDraw = (edge: BrainEdge): boolean => {
			...
			const sourcePinned = Boolean(this.plugin.settings.pinnedNodePositions[edge.source]);
			const targetPinned = Boolean(this.plugin.settings.pinnedNodePositions[edge.target]);
			if ((!enabled[sourceNode.region] && !sourcePinned) || (!enabled[targetNode.region] && !targetPinned)) return false;
	...
	private drawNodes(): void {
		...
		for (const sn of this.screenNodes) {
			...
			const isPinned = Boolean(this.plugin.settings.pinnedNodePositions[sn.node.file.path]);
			...
			if (sn.node.isHub) {
				const pulse = this.plugin.settings.idlePulse ? 0.4 + 0.5 * (0.5 + 0.5 * Math.sin(this.pulsePhase * Math.PI * 2)) : 0.55;
			...
			if (isPinned) {
				...
				ctx.setLineDash([2, 2]);
				ctx.beginPath();
				ctx.arc(sn.x, sn.y, sn.radius + 4.5, 0, Math.PI * 2);
				ctx.stroke();
				ctx.setLineDash([]);
			}
