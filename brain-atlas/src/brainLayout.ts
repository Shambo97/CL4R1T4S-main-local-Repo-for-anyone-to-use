...
import { BrainRegion, PinnedPosition } from "./settings";
 ...
 *  A node with a saved pinned position skips all of that and is placed exactly where the user dragged it,
 *  free to sit anywhere in the silhouette regardless of its assigned lobe.
 *  Repulsion is skipped past `relaxCap` unpinned nodes since it's O(n^2) per lobe and canvas redraw is
 *  already the bottleneck at that scale. */
export function layoutBrainNodes(
	nodes: BrainNode[],
	pinned: Record<string, PinnedPosition>,
	relaxCap = 260
	...
	for (const node of nodes) {
		const pin = pinned[node.file.path];
		if (pin) {
			positions.set(node.file.path, { x: pin.x, y: pin.y });
			continue;
		}
